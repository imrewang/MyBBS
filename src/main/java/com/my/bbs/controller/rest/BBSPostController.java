...
public class BBSPostController {
                             ...
                             @RequestParam(value = "commentPage", required = false, defaultValue = "1") Integer commentPage)
    {
        ...
        // 帖子内容
        BBSPost bbsPost = bbsPostService.getBBSPostForDetail(postId);//获取详情&浏览数加1
        if (bbsPost == null) {
            return "error/error_404";
        }
        ...
        // 本周热议的帖子
        request.setAttribute("hotTopicBBSPostList", bbsPostService.getHotTopicBBSPostList());//近期热议帖子列表

        // 评论数据
        PageResult commentsPage = bbsPostCommentService.getCommentsByPostId(postId, commentPage);//详情页评论列表
    ...
    public String editPostPage(HttpServletRequest request,
                               @PathVariable(value = "postId") Long postId)
    {
        BBSUser bbsUser = (BBSUser) request.getSession().getAttribute(Constants.USER_SESSION_KEY);
        List<BBSPostCategory> bbsPostCategories = bbsPostCategoryService.getBBSPostCategories();//获取分类列表
        if (CollectionUtils.isEmpty(bbsPostCategories)) {
            return "error/error_404";
        }
        //将分类数据封装到request域中
        ...
        BBSPost bbsPost = bbsPostService.getBBSPostById(postId);//获取详情
        if (bbsPost == null) {
            return "error/error_404";
        }
                          ...
                          HttpSession httpSession)
    {
        ...
        if (bbsPostService.savePost(bbsPost) > 0) {//保存帖子
    ...
    public Result delPost(@PathVariable("postId") Long postId,
        ...
        if (bbsPostService.delBBSPost(bbsUser.getUserId(), postId) > 0) {//删除帖子
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult("请求失败，请检查参数及账号是否有操作权限");
        }
                           ...
                           HttpSession httpSession)
    {
        ...
        if (bbsPostService.updateBBSPost(temp) > 0) {//修改帖子
            httpSession.removeAttribute(Constants.VERIFY_CODE_KEY);//清空session中的验证码信息
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult("请求失败，请检查参数及账号是否有操作权限");
