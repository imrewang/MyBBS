...
public class BBSUserController {
    ...
    {
        //基本用户信息
        BBSUser bbsUser = bbsUserService.getUserById(userId);//获取用户详情
        if (bbsUser == null) {
            return "error/error_404";
        }

        //近期发布的帖子
        List<BBSPost> recentPostList = bbsPostService.getRecentPostListByUserId(userId);//根据userId获取最近发帖列表
    ...
    public String myCenterPage(HttpServletRequest request) {
        ...
        //我发的贴
        List<BBSPost> myBBSPostList = bbsPostService.getMyBBSPostList(currentUser.getUserId());//根据userId查询发布的所有帖子
        ...
        //我收藏的贴
        List<BBSPost> collectRecords = bbsPostCollectService.getCollectRecordsByUserId(currentUser.getUserId());//获取收藏的帖子列表
    ...
    public Result updateInfo(@RequestParam("nickName") String nickName,
        ...
        if (bbsUserService.updateUserInfo(bbsUser, httpSession)) {//修改用户信息
    ...
    public Result updateHeadImg(@RequestParam("userHeadImg") String userHeadImg,
        ...
        if (bbsUserService.updateUserHeadImg(bbsUser, httpSession)) {//修改用户头像
            Result result = ResultGenerator.genSuccessResult();
            return result;
        } else {
            Result result = ResultGenerator.genFailResult("修改失败");
    ...
    public Result passwordUpdate(HttpServletRequest request, @RequestParam("originalPassword") String originalPassword,
        ...
        BBSUser currentUser = (BBSUser) request.getSession().getAttribute(Constants.USER_SESSION_KEY);
        if (bbsUserService.updatePassword(currentUser.getUserId(), originalPassword, newPassword)) {//修改密码
    ...
    public Result register(@RequestParam("loginName") String loginName,
        ...
        String registerResult = bbsUserService.register(loginName, password, nickName);//用户注册
        //注册成功
